...
class Network:
    def __init__(self):
        ...
        self.link_enter_leave_df = pd.DataFrame(columns = ['agent_id', 'current_link', 'next_link', 'current_link_enter_time', 'current_link_leave_time'])
    ...
    def prepare_links(self, links_df, virtual_nodes_df):
        ...
        self.links = links_df[['link_id', 'start_node_id', 'end_node_id', 'geometry', 'link_type']].copy()
        self.links['road_type'] = self.links['link_type'].copy() # link.csv has a column named link_type, rename it as road_type
        ...
        try: 
            self.links['maxmph'] = links_df['maxmph']
            #self.links['maxmph'] = self.links['maxmph'].apply(lambda x: 30 if x<=30 else x) # the minimum maxmph is 30 miles/h
        except KeyError:
            self.links['maxmph'] = 25 # the minimum maxmph is 30 miles/h
        ...
        virtual_links_df['road_type'] = 'virtual'
        ...
        self.links['storage_add_times'] = 0 # default is 0
        self.links['rtt'] = 0
        self.links['rtt'] = np.where(self.links['rtt'] < self.links['fft'], self.links['fft'], self.links['rtt']) # real average travel time, updated every 10 mins, if equals 0, let it be the fft
        self.links['rtt'] = np.where(self.links['road_type'] == 'motorway', self.links['rtt']*0.95, self.links['rtt'])
        self.links['rtt'] = np.where(self.links['road_type'] == 'primary', self.links['rtt']*0.96, self.links['rtt'])
        self.links['rtt'] = np.where(self.links['road_type'] == 'secondary', self.links['rtt']*0.97, self.links['rtt'])
        self.links['rtt'] = np.where(self.links['road_type'] == 'tertiary', self.links['rtt']*0.98, self.links['rtt'])
        #self.links['rtt'] = self.links['rtt'] + 0.01 * self.links['length']/self.links['lanes']

        self.links = self.links[['link_id', 'start_node_id', 'end_node_id', 'link_type', 'road_type', 'length', 'lanes', 'maxmph', 'fft', 'rtt', 'capacity', 'storage', 'geometry','storage_add_times']]
    ...
    def prepare_network(self, nodes_df, links_df, ods_df):
        ...
        self.g = interface.from_dataframe(self.links[self.links['link_type']=='real'], 'start_node_id', 'end_node_id', 'rtt')
    ...
    def find_next_link(self, agent, agent_routes, just_loaded=False):
        ...
        agent_cl_enid = int(getattr(agent, 'cl_enid'))
        ### just loaded
        if just_loaded:
        ...
        if agent_cl_enid == int(getattr(agent, 'destin_nid')):
            return ['vl_sink', agent_cl_enid, 'sink_node', 0.1, 1e8, 1e8/3.6, None, None, None, None]
        #print('current_agent_id:{}, current_cl_enid:{}, current_agent_route:{}'.format(agent_id, agent_cl_enid, agent_routes[agent_id]))
        agent_nl_enid = agent_routes[agent_id][agent_cl_enid] #agent_routes[agent_id][agent_cl_enid]
        agent_nl = self.link_nid_dict[(agent_cl_enid, agent_nl_enid)] #self.link_nid_dict[(agent_cl_enid, agent_nl_enid)]
        ...
        if agent_nl_enid == int(getattr(agent, 'destin_nid')):
        ...
        #print('agent_id:{}, cl_enid: {}, nl_enid: {}, agent_routes:{}'.format(agent_id, agent_cl_enid, agent_nl_enid, agent_routes[agent_id]))
    ...
    def run_link_model(self, agents=None, t=None):
        ...
        if t % 600 == 0:
            ...
            self.links['rtt'] = self.links['link_id'].map(link_travel_time_dict).fillna(0)
            self.links['rtt'] = np.where(self.links['rtt'] < self.links['fft'], self.links['fft'], self.links['rtt'])
            self.links['rtt'] = np.where(self.links['road_type'] == 'motorway', self.links['rtt']*0.96, self.links['rtt'])
            self.links['rtt'] = np.where(self.links['road_type'] == 'primary', self.links['rtt']*0.97, self.links['rtt'])
            self.links['rtt'] = np.where(self.links['road_type'] == 'secondary', self.links['rtt']*0.98, self.links['rtt'])
            self.links['rtt'] = np.where(self.links['road_type'] == 'tertiary', self.links['rtt']*0.99, self.links['rtt'])
            self.g = interface.from_dataframe(self.links[self.links['link_type']=='real'], 'start_node_id', 'end_node_id', 'rtt') # update g

        current_time_storage_remain = (self.links['storage'] - (self.links['run'] * 8 + self.links['queue'] * 5)).values
    ...
    def run_node_model(self, agents=None, t=None, special_nodes=None):
        ...
        if len(special_nodes) != 0:
            ...
            go_agents_df['default_priority'] = np.where(go_agents_midx.isin(
                special_nodes), 1, go_agents_df['default_priority'])
        ...
        go_agents_df = go_agents_df[['agent_id', 'current_link', 'next_link', 'current_link_enter_time']]
...
class Agents:
    ...
    def update_route_info(self, reroute_agents, network, just_loaded=False):
        ...
        for agent in reroute_agents.itertuples():
            ...
            if (int(getattr(agent, 'cl_enid')) == int(getattr(agent, 'destin_nid'))) & (just_loaded == False):
                noneedreroute_list.append(agent_id)
                #print('noneedreroute_list include {}'.format(agent_id))
                continue
            ...
            if len(route)==0: 
                ...
                print('no route for {}, cl_enid {}, destin_nid {}'.format(agent_id, int(getattr(agent, 'cl_enid')), int(getattr(agent, 'destin_nid'))))
            else:
                agent_clnl_update[agent_id] = network.find_next_link(agent, self.agent_routes, just_loaded)
        if len(unarrived_list)>0:
            self.agents.loc[self.agents['agent_id'].isin(unarrived_list), 'agent_status'] = -2
            reroute_agents = reroute_agents.loc[~reroute_agents['agent_id'].isin(unarrived_list), :].copy()
        if len(noneedreroute_list) > 0:
            reroute_agents = reroute_agents.loc[~reroute_agents['agent_id'].isin(noneedreroute_list), :].copy()
        if len(agent_clnl_update) == 0:
            return

        # because the function of reroutes messes up the order, we cannot directly use pd.DataFrame(reroute_agents['agent_id'].map(xxx)
        self.agents.loc[(self.agents['agent_id'].isin(reroute_agents['agent_id'])), 
                            ...
                            ]] = pd.DataFrame(self.agents.loc[(self.agents['agent_id'].isin(reroute_agents['agent_id'])),
                                'agent_id'].map(agent_clnl_update).to_list()).values
    ...
    def reroutes(self, network=None, t=None, reroute_frequency=None):
        ...
        if reroute_frequency is None:
            ...
            if (t >=1) & (t%reroute_frequency == 0):
                ### staggered rerouting
                reroute_agents = self.agents[(self.agents['agent_status'].isin([1, 2]))].sample(frac=0.05)
                if len(reroute_agents) == 0:
                    return
...
class Simulation():
    def __init__(self):
        self.agents = None
        self.network = None
        self.special_nodes = [] #set special nodes to give priority to the agents in the roundabouts
        self.loops = {} # record the time when each loop exists
    ...
    def find_loops(self, t, agents_df, links_df):
        self.special_nodes = []
        loop_agents_df = agents_df.copy()
        loop_agents_df = loop_agents_df[loop_agents_df['agent_status'].isin([1,2])]
        loop_agents_df['current_link'] = loop_agents_df['current_link'].astype('str')
        loop_agents_df['current_link_id_first_letter'] = loop_agents_df['current_link'].apply(lambda x: x[0])
        loop_agents_df = loop_agents_df.loc[(loop_agents_df['current_link_id_first_letter'] != 'v'), :]
        loop_agents_df['current_link'] = loop_agents_df['current_link'].astype('float')

        loop_agents_df['next_link'] = loop_agents_df['next_link'].astype('str')
        loop_agents_df['next_link_id_first_letter'] = loop_agents_df['next_link'].apply(lambda x: x[0])
        loop_agents_df = loop_agents_df.loc[loop_agents_df['next_link_id_first_letter'] != 'v',  :]
        loop_agents_df['next_link'] = loop_agents_df['next_link'].astype('float')

        loop_agents_df = loop_agents_df[['agent_id','current_link', 'next_link']]
        loop_agents_df.reset_index(drop = True, inplace = True)

        link_total = np.unique(loop_agents_df['current_link'].values)

        next_link_dict = [{link_tmp: np.unique(loop_agents_df.loc[loop_agents_df['current_link'] == 
                        link_tmp, 'next_link'].values).tolist()} for link_tmp in link_total]
        DG = DiGraph()
        DG.add_edges_from([(int(getattr(agent_tmp, 'current_link')), int(getattr(agent_tmp, 'next_link'))) for agent_tmp in loop_agents_df.itertuples()])
        loop_list = list(simple_cycles(DG))
        if len(loop_list) == 0:
            print('check: currently, there is no loop in the network')
            return
        else:
            loop_link_total= []
            for loop_tmp in loop_list:
                loop_tmp_storage_remain = links_df.loc[links_df['link_id'].isin(loop_tmp),'storage_remain']
                if len(loop_tmp_storage_remain[loop_tmp_storage_remain <= 0]) == len(loop_tmp_storage_remain):
                    loop_link_total =  loop_link_total + loop_tmp
                    if t in self.loops.keys():
                        self.loops[t].append(loop_tmp)
                    else:
                        self.loops[t] = [loop_tmp]
                    print('check: there exists one deadlock loop')
                [self.special_nodes.append(tuple((loop_tmp[i], loop_tmp[i+1]))) for i in range(len(loop_tmp)-1)]
                self.special_nodes.append(tuple((loop_tmp[-1],loop_tmp[0])))

            loop_link_total = np.unique(loop_link_total).tolist()
            if len(loop_link_total) == 0:
                print('check: currently, there is no loop in the network')
                return
            else:
                self.network.links['storage_remain'] = np.where(self.network.links['link_id'].isin(loop_link_total), self.network.links['storage_remain'] + 8, self.network.links['storage_remain'])
                self.network.links['storage_add_times'] = np.where(self.network.links['link_id'].isin(loop_link_total), self.network.links['storage_add_times'] + 1, 
                                                            self.network.links['storage_add_times'])
    ...
    def run_one_step(self, t, reroute_frequency=None):
        ...
        #if (t > 1) & (t % 600 == 0):
        #    self.find_loops(t=t, agents_df = self.agents.agents, links_df = self.network.links)


        self.agents.agents = self.network.run_node_model(agents=self.agents, t=t, special_nodes=self.special_nodes)
