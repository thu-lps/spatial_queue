...
class Network:
    ...
    def prepare_links(self, links_df, virtual_nodes_df):
        ...
        self.links['fft'] = self.links['length']/(self.links['maxmph']*1609/3600) # free flow time
        ...
        self.links = self.links[['link_id', 'start_node_id', 'end_node_id', 'link_type', 'length', 'lanes', 'maxmph', 'fft', 'capacity', 'storage', 'geometry', 'rtt']]
    ...
    def node_movements(self):
        ...
        for node in self.nodes[self.nodes['node_type']=='real'].itertuples():
            ...
            for il in incoming_links[node_id]:
                for ol in outgoing_links[node_id]: #应该要增加 vl_sink这个node
                    ### virtual incoming links not considered here
    ...
    def run_link_model(self, agents=None, t=None):
        ...
        if t % 900 == 0:
           link_travel_time_dict = ((t - agents_df.loc[agents_df['agent_status'].isin([1,2])].groupby('current_link')['current_link_enter_time'].sum()) /
                                    agents_df.loc[agents_df['agent_status'].isin([1,2])].groupby('current_link').size()).to_dict()
        ...
        self.links['storage_remain'] = (self.links['storage'] - (self.links['run'] + self.links['queue'])*8).values
    ...
    def run_node_model(self, agents=None, t=None, special_nodes=None):
        ...
        if 122230 in go_ids:
            print('122230更新后的状态:',agents_df.loc[(agents_df['agent_id']==122230), ['current_link', 'cl_snid', 'cl_enid']])
...
class Agents:
    ...
    def update_route_info(self, reroute_agents, network, just_loaded=False):
        ...
        if len(agent_clnl_update) == 0: #检查agent_clnl_current是不是在vl_link上
            return
    ...
    def reroutes(self, network=None, t=None, reroute_frequency=None):
        ...
        if reroute_frequency is None:
            ...
            if (t >=1) & (t%reroute_frequency == 0):
                ### staggered rerouting
                reroute_agents = self.agents.loc[self.agents['agent_status'].isin([1, 2]),:]
                reroute_agents = reroute_agents.sample(frac=0.2).sort_values(by = 'agent_id', ascending = True)
                #print(reroute_agents)
            ...
            ###        ((self.agents['agent_id']%reroute_frequency==t) | (self.agents['agent_id']%reroute_frequency==reroute_frequency - t))]
            ###    if len(reroute_agents) == 0:
            ###        return
                ### update routes
            ###    self.update_route_info(reroute_agents, network)
...
#        if t >=1:
#            print(self.agents.agents.loc[self.agents.agents['agent_id'] == 122791, ['origin_nid','destin_nid','cl_enid','nl_enid']]) 
