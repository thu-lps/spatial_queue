...
class Network:
    def __init__(self):
        ...
        #self.link_previous_sr_300 = None
    ...
    def prepare_links(self, links_df, virtual_nodes_df):
        ...
        #self.links['sequence'] = np.arange(self.links.shape[0])
        self.links['storage_add_times'] = 0 # default is 0
        #self.link_previous_sr_300 = np.ones((self.links.shape[0], 300)) #record the link storage_remain in the previous 600 seconds,default is 1
    ...
    def run_link_model(self, agents=None, t=None):
        ...
        #position = [i for i in range(self.link_previous_sr_300.shape[0]) if len(self.link_previous_sr_300[i][self.link_previous_sr_300[i]<=0]) == len(self.link_previous_sr_300[i])]      
        current_time_storage_remain = (self.links['storage'] - (self.links['run'] + self.links['queue'])*8).values
        # only change the storage remain which is between -8 and 0, -> ensure that not many cars run/queue in this link to let this link destroys the congestion itself.
        #position1 = np.where((current_time_storage_remain <= 0) & (current_time_storage_remain >-8))[0].tolist()
        #position = list(set(position) & set(position1))
        
        #current_time_storage_remain[position] = 8 # only one car can enter
        #self.link_previous_sr_300[:,1:] = self.link_previous_sr_300[:,:-1]
        #self.link_previous_sr_300[:,0] = current_time_storage_remain
