...
def write_all_link_outputs_vect(simulation, save_path):
    link_output = simulation.network.links.loc[
        simulation.network.links['link_type'].isin(['real', 'virtual']), 
        ['link_id', 'length','rtt', 'geometry',]].copy()
...
arrival_output_path = 'traffic_outputs/{}/with_bg/t_stats_onlybg/arrivals_{}_onlybg.csv'.format(case, scenario_name)
...
for t in range(t_end):
    ...
    if t%100 == 0:
        if bg_exist == 1:
            link_output_path = 'traffic_outputs/{}/with_bg/link_stats_onlybg/l{}_at_{}_onlybg.csv'.format(case, scenario_name, t)
            agent_output_path = 'traffic_outputs/{}/with_bg/agent_stats_onlybg/{}_at_{}_onlybg.csv'.format(case, scenario_name, t)
    ...
    if t%100 == 0:
        if not arrival_counts_vect(t, simulation_vect, arrival_output_path):
            write_loops(simulation_vect, 'traffic_outputs/{}/with_bg/loop_onlybg/loops_add_srsn_reroute_rtt.xlsx'.format(case))
            write_all_link_outputs_vect(simulation_vect, 'traffic_outputs/{}/with_bg/link_stats_onlybg/l{}_at_{}_all_links.csv'.format(case, scenario_name, t))
            break
