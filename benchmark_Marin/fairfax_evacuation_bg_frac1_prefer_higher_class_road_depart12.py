...
od_df['agent_id'] = np.arange(400000, 400000 + len(od_df))

od_df['departure_time'] = od_df.groupby(by = ['origin_nid', 'destin_nid'],sort=False).cumcount()
od_df['departure_time'] = od_df['departure_time'].apply(lambda x: x*10) + 3600 * (depart - 6) # departure interval is 10 seconds
...
if bg_exist == 1:
    background_od = pd.read_csv('traffic_inputs/{}/background_ods_day_for_Marin_new_change_marin_side_od_add_afternoon.csv'.format(case,case), float_precision = "round_trip")
