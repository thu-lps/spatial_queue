...
od_df['departure_time'] = od_df['departure_time'].apply(lambda x: x*30) # departure interval is 30 seconds
...
t_end = 3600 * 20
if bg_exist == 1:
    arrival_output_path = 'traffic_outputs/{}/with_bg/t_stats_final/arrivals_{}_bg_1_total.csv'.format(case, scenario_name)
    arrival_output_path_1 = 'traffic_outputs/{}/with_bg/t_stats_final/arrivals_{}_bg_1_evacuation.csv'.format(case, scenario_name)
else:
...
for t in range(t_end):
    ...
    if t%100 == 0:
        if not arrival_counts_vect(t, simulation_vect, arrival_output_path,arrival_output_path_1):
            write_loops(simulation_vect, 'traffic_outputs/{}/with_bg/loop_final/loops_add_srsn_reroute_rtt.xlsx'.format(case))
