...
import model.spatial_queue_array_add_storage_remain_special_node_reroute_rtt_random as sq_vect
...
for seed_number in range(21,41):
    print('current_seed_number:{}'.format(seed_number))
    ...
    simulation_vect.initialize_simulation(nodes_df, links_df, od_df,seed_number)
    
    ### specify some parameters
    scenario_name = '{}_vect'.format(case)
    t_end = 3600 * 20
    if bg_exist == 1:
        arrival_output_path = 'traffic_outputs/{}/with_bg/t_stats_add_srsn_reroute_rtt/arrivals_{}_bg_1_real_departure_time_total_random{}.csv'.format(case, scenario_name,seed_number)
        arrival_output_path_1 = 'traffic_outputs/{}/with_bg/t_stats_add_srsn_reroute_rtt/arrivals_{}_bg_1_real_departure_time_evacuation_random{}.csv'.format(case, scenario_name,seed_number)
    else:
    ...
    for t in range(t_end):
        ...
        if t%100 == 0:
            if not arrival_counts_vect(t, simulation_vect, arrival_output_path,arrival_output_path_1):
                write_loops(simulation_vect, 'traffic_outputs/{}/with_bg/loop_add_srsn/loops_add_srsn_reroute_rtt_random{}.xlsx'.format(case,seed_number))###
                break
