...
links_df.loc[links_df['link_id'].isin([453,15382, 31975, 10368, 4969, 13113, 9924, 30445, 22430, 4456, 23103, 189, 
    15153, 17738, 11631, 6037, 17671, 12217, 178, 223, 8627, 14269, 30469, 232, 3728, 16782]), 'lanes'] += 1
...
arrival_output_path = 'traffic_outputs/{}/sensitivity_analysis/t_stats_widen_Route6/arrivals_{}_bg_1_real_departure_time_total.csv'.format(case, scenario_name)
arrival_output_path_1 = 'traffic_outputs/{}/sensitivity_analysis/t_stats_widen_Route6/arrivals_{}_bg_1_real_departure_time_evacuation.csv'.format(case, scenario_name)
...
for t in range(t_end):
    ...
    if t%100 == 0:
        if bg_exist == 1:
            link_output_path = 'traffic_outputs/{}/sensitivity_analysis/link_stats_widen_Route6/l{}_at_{}_bg_1_real_departure_time.csv'.format(case, scenario_name, t)
            agent_output_path = 'traffic_outputs/{}/sensitivity_analysis/agent_stats_widen_Route6/{}_at_{}_bg_1_real_departure_time_agent.csv'.format(case, scenario_name, t)
    ...
    if t%100 == 0:
        if not arrival_counts_vect(t, simulation_vect, arrival_output_path,arrival_output_path_1):
            write_loops(simulation_vect, 'traffic_outputs/{}/sensitivity_analysis/loop_widen_Route6/loops_add_srsn_reroute_rtt.xlsx'.format(case))
            break
