...
od_df['agent_id'] = np.arange(250000, 250000 + len(od_df))
### these are done for the vectorized implementation
background_od = pd.read_csv('traffic_inputs/{}/background_ods_day_for_Marin_add_afternoon.csv'.format(case,case), float_precision = "round_trip")
...
od_df.loc[:, 'departure_hour'] = od_df.loc[:, 'departure_hour'].fillna(departure_hour)
...
def arrival_counts_vect(t, simulation, save_path, save_path_1):
    ...
    if arrival_cnts == need_arrival_cnts:
        print("all evacuation agents arrive at destinations at time {} seconds.".format(t))
        return False
...
arrival_output_path = 'traffic_outputs/{}/sensitivity_analysis/t_stats_depart{}/arrival_{}_total.csv'.format(case,departure_hour,scenario_name)
arrival_output_path_1 = 'traffic_outputs/{}/sensitivity_analysis/t_stats_depart{}/arrival_{}_evacuation.csv'.format(case,departure_hour,scenario_name)
...
for t in range(t_end):
    ...
    #if t%100 == 0:
    #    link_output_path = 'traffic_outputs/{}/sensitivity_analysis/link_stats_depart{}/{}_at_{}_link.csv'.format(case,departure_hour,scenario_name, t)
    #    agent_output_path = 'traffic_outputs/{}/sensitivity_analysis/agent_stats_depart{}/{}_at_{}_agent.csv'.format(case,departure_hour,scenario_name, t)
    ...
    if t%100 == 0:
        if not arrival_counts_vect(t, simulation_vect, arrival_output_path,arrival_output_path_1):
            #write_loops(simulation_vect, 'traffic_outputs/{}/sensitivity_analysis/loop_depart{}/loops_add_srsn_reroute_rtt.xlsx'.format(case,departure_hour))
            #write_all_link_outputs_vect(simulation_vect, 'traffic_outputs/{}/sensitivity_analysis/link_stats_depart{}/l{}_at_{}_all_links.csv'.format(case,departure_hour,scenario_name,t))
            break
