...
od_df = pd.read_csv('traffic_inputs/{}/{}_ods_day.csv'.format(case,case), float_precision = "round_trip")
...
if bg_exist == 1:
    ...
    background_od = pd.read_csv('traffic_inputs/{}/background_ods_day_for_Marin.csv'.format(case,case), float_precision = "round_trip")
...
def arrival_counts_vect(t, simulation, save_path, save_path_1):
    arrival_cnts = np.sum((simulation.agents.agents['agent_status'] == -1) & (simulation.agents.agents['destin_nid'] == 21332))
    need_arrival_cnts = np.sum(simulation.agents.agents['destin_nid'] == 21332)
