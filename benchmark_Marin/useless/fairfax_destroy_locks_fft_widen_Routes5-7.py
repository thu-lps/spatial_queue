...
import model.spatial_queue_array_add_storage_remain_special_node as sq_vect
...
links_df.loc[links_df['link_id'].isin([5833, 4402, 38005, 13318, 4834, 2940, 4849, 1247, 17681, 15372, 453,15382, 31975, 
    10368, 4969, 13113, 9924, 30445, 22430, 4456, 23103, 189, 15153, 17738, 11631, 6037, 17671, 12217, 178, 223, 8627, 
    14269, 30469, 232, 3728, 16782, 4865, 4845, 534, 12939, 2206, 8297, 31477, 31136, 16267, 5822, 9883, 30654, 31262, 
    9805, 37822, 31288, 7510, 11522, 16122, 29552, 8169, 10578]), 'lanes'] += 1
...
arrival_output_path = 'traffic_outputs/{}/sensitivity_analysis/t_stats_add_srsn_widen_Routes5-7/arrivals_{}_bg_1_real_departure_time_total.csv'.format(case, scenario_name)
arrival_output_path_1 = 'traffic_outputs/{}/sensitivity_analysis/t_stats_add_srsn_widen_Routes5-7/arrivals_{}_bg_1_real_departure_time_evacuation.csv'.format(case, scenario_name)
...
for t in range(t_end):
    ...
    if t%100 == 0:
        link_output_path = 'traffic_outputs/{}/sensitivity_analysis/link_stats_add_srsn_widen_Routes5-7/l{}_at_{}_bg_1_real_departure_time.csv'.format(case, scenario_name, t)
        agent_output_path = 'traffic_outputs/{}/sensitivity_analysis/agent_stats_add_srsn_widen_Routes5-7/{}_at_{}_bg_1_real_departure_time_agent.csv'.format(case, scenario_name, t)
    ...
    if t%100 == 0:
        if not arrival_counts_vect(t, simulation_vect, arrival_output_path,arrival_output_path_1):
            write_loops(simulation_vect, 'traffic_outputs/{}/sensitivity_analysis/loop_add_srsn_widen_Routes5-7/loop_add_srsn.xlsx'.format(case))
            #write_all_link_outputs_vect(simulation_vect, 'traffic_outputs/{}/link_stats_add_srsn_widen_Route5/l{}_at_{}_all_links.csv'.format(case, scenario_name, t))
            break

    #if t % 1000 == 0:
    #    write_all_link_outputs_vect(simulation_vect, 'traffic_outputs/{}/link_stats_add_srsn/l{}_at_{}_all_links.csv'.format(case, scenario_name, t))
