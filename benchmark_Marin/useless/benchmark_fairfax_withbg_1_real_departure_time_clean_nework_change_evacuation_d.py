...
import model.spatial_queue_array_edited as sq_vect
...
if bg_exist == 1:
    ...
    background_od = pd.read_csv('traffic_inputs/{}/background_ods_day_for_Marin_new.csv'.format(case,case), float_precision = "round_trip")
...
def write_link_outputs_vect(simulation, save_path):
    ...
    link_output['link_id'] = link_output['link_id'].astype('str')
    link_output['link_id'] = link_output['link_id'].apply(lambda x: x.split('.')[0] if '.' in x else x)
    link_output = link_output[link_output['link_id'].isin(['18737', '3217', '37492', '31486', '10866', '39748', '37947', '13707', '37841', '39708', '31859', '37190', '20534'])]
...
def write_agent_evac_outputs_vect(simulation, save_path):
    ...
    agent_output['current_link'] = agent_output['current_link'].astype('str')
    agent_output['next_link'] = agent_output['next_link'].astype('str')
    agent_output['current_link'] = agent_output['current_link'].apply(lambda x: x.split('.')[0] if '.' in x else x)
    agent_output['next_link'] = agent_output['next_link'].apply(lambda x: x.split('.')[0] if '.' in x else x)
    agent_output = agent_output[agent_output['current_link'].isin(['18737', '3217', '37492', '31486', '10866', '39748', '37947', '13707', '37841', '39708', '31859', '37190', '20534'])]
...
t_end = 3600 * 6
if bg_exist == 1:
    arrival_output_path = 'traffic_outputs/{}/with_bg/t_stats/arrivals_{}_bg_1_real_departure_time_total.csv'.format(case, scenario_name)
    arrival_output_path_1 = 'traffic_outputs/{}/with_bg/t_stats/arrivals_{}_bg_1_real_departure_time_evacuation.csv'.format(case, scenario_name)
else:
    arrival_output_path = 'traffic_outputs/{}/no_bg/t_stats/arrivals_{}_bg_1_real_departure_time.csv'.format(case, scenario_name)
...
for t in range(t_end):
    ...
    if t%100 == 0:
        if bg_exist == 1:
            link_output_path = 'traffic_outputs/{}/with_bg/link_stats/l{}_at_{}_bg_1_real_departure_time.csv'.format(case, scenario_name, t)
            agent_output_path = 'traffic_outputs/{}/with_bg/agent_stats/{}_at_{}_bg_1_real_departure_time_agent.csv'.format(case, scenario_name, t)
        else:
            link_output_path = 'traffic_outputs/{}/no_bg/link_stats/l{}_at_{}_bg_1_real_departure_time.csv'.format(case, scenario_name, t)
