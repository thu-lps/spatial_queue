...
agent_evac = od_df.loc[od_df['destin_nid'] == 192503, 'agent_id'].values #记录evac_agent信息
...
def write_agent_evac_outputs_vect(simulation, save_path):
    agent_output = simulation.agents.agents.loc[simulation.agents.agents['agent_id'].isin(agent_evac), ['current_link', 'cl_lanes', 'cl_capacity', 'next_link', 'nl_lanes', 'nl_capacity', 'current_link_enter_time', 'agent_status', 'origin_nid', 'destin_nid']].copy()
...
t_end = 360000
if bg_exist == 1:
    arrival_output_path = 'traffic_outputs/{}/with_bg/t_stats/arrivals_{}_bg_1_agent.csv'.format(case, scenario_name)
else:
    arrival_output_path = 'traffic_outputs/{}/no_bg/t_stats/arrivals_{}_bg_1.csv'.format(case, scenario_name)
...
for t in range(t_end):
    ...
    if t%100 == 0:
        if not arrival_counts_vect(t, simulation_vect, arrival_output_path):
            break
        if bg_exist == 1:
            link_output_path = 'traffic_outputs/{}/with_bg/link_stats/l{}_at_{}_bg_1_agent.csv'.format(case, scenario_name, t)
            agent_output_path = 'traffic_outputs/{}/with_bg/agent_stats/{}_at_{}_bg_1_agent.csv'.format(case, scenario_name, t)
        else:
            link_output_path = 'traffic_outputs/{}/no_bg/link_stats/l{}_at_{}_bg_1.csv'.format(case, scenario_name, t)
